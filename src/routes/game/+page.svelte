...
<div class="w-full flex flex-row">
	{#if !gameStart}
		...
		<div class="px-4 py-10 w-2/5  flex flex-col justify-between">
			<div class="space-y-4">
				<p class="text-gray-700 text-xl">Bienvenue dans une journée ordinaire !</p>
				<p>
					Vous vous réveillez dans un lit confortable dans votre chambre, prêt à affronter tout ce
					que la journée pourrait vous apporter.
				</p>
				<p>
					Vous sortez du lit, vous habillez et descendez pour le petit-déjeuner. Vous appréciez le
					petit déjeuner avec votre famille, puis commencez votre journée.
				</p>
				<p>
					Vous vous préparez pour l'école et sortez. Vous avez une journée chargée de cours, d'amis
					et d'activités devant vous, et vous avez hâte de commencer. Aujourd'hui va être une
					journée incroyable !
				</p>
				...
				<label for="p-sexe" class="block mb-2 text-sm font-medium text-gray-900"
					>Sexe de votre partenaire</label
				>
				<select
					id="p-sexe"
					class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:focus:border-blue-500"
	...
	{:else}
		<img
			src={gameData[gameState].img}
			class="z-10 w-full h-screen object-cover object-bottom absolute"
			alt=""
		/>
		<div class="z-50 w-full h-screen flex flex-col justify-end pb-8 px-32">
			<div class="bg-white/95 border rounded p-4 text-xl">
			...
			<div class="space-x-4 flex my-8">
				{#if gameState > 6}
					<a
						href="/game/result"
						class="flex-1 bg-white/70 p-4 rounded text-center hover:bg-white/75 transition-all duration-300"
					>
						Voir Mon score
					</a>
				{:else}
					{#each gameData[gameState].actions as action}
						<button
							on:click={continueGame(action.state)}
							class="flex-1 text-xl bg-white/70 p-4 rounded hover:bg-white/75 transition-all duration-300"
						>
							{action.text}
						</button>
					{/each}
